...
def main():
    ...
    parser.add_argument(
        "--input_format",
        default="typed_entity_marker_punct",
        type=str,
    ...
    parser.add_argument(
        "--test_batch_size", default=32, type=int, help="Batch size for testing."
    )
    parser.add_argument(
        "--learning_rate",
        default=3e-5,
    ...
    if train_pkl.exists():
        train_features = pickle.loads(train_pkl.read_bytes())
    else:
        train_features = processor.read(train_file)

    dev_pkl = Path("./tmp/dev.pkl")
    if dev_pkl.exists():
        dev_features = pickle.loads(dev_pkl.read_bytes())
    else:
        dev_features = processor.read(dev_file)

    test_pkl = Path("./tmp/test.pkl")
    if test_pkl.exists():
        test_features = pickle.loads(test_pkl.read_bytes())
    else:
